 ...
 * coexist. It hides again once `#contact` intersects, so email and footer
 * LinkedIn stay tappable. Server-rendered hidden; the observer reveals it
 * after hydration when the button is not in view.
 *
...
export function ResumeBar({ cta, watchId }: Props) {
  const isHeroCtaVisible = useHeroCtaVisibility(
    watchId,
    `0px 0px -${BAR_HEIGHT_PX}px 0px`,
  );
  const isContactVisible = useHeroCtaVisibility(footerView.section.id, "0px");
  const visible = shouldShowResumeBar(isHeroCtaVisible, isContactVisible);
